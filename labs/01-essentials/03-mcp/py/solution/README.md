...
1. Change to the solution folder in a new terminal window:
...
## Connect to the _local_ MCP server from a client/host
...
## Connect to your _remote_ MCP server function app from a client
...
The function code for the `get_snippet` and `save_snippet` endpoints are defined in the Python files in the `solution` directory. The MCP function annotations expose these functions as MCP Server tools.
